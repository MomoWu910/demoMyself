...
![Cocos 賭場大廳：六張卡片——三款能玩、百家樂的 shader 對照版、兩張佔位；左上角走得回雲朵樂園](docs/screenshots/cocos-casino.jpg)

用 **Cocos Creator 3.8** 做的一個大廳加三款遊戲。它們有自己的引擎與建置流程，沒辦法併進這裡的
webpack，所以原始碼在獨立的 `cocos-lab`（private），成品 build 出來放在本站：

**[→ 賭場大廳](https://demomyself-casino.pages.dev/)**（百家樂 · 輪盤 · 老虎機，**這一款在 Cloudflare Pages**，原因見下面那張對照表）——
...
- **百家樂 · SHADER FX** — 同一個場景加上三個自訂 shader（籌碼金屬反光、輸家褪色、清桌燃燒），右上角面板可以當場切換「關／正確／錯誤示範」並即時看 DrawCall。燃燒用的 noise 是 Shader Lab 那支 Dissolve 一字不改搬進 Cocos 的 `.effect`。
...
**幾組量出來的數字**（驗證腳本都在 repo 裡，可重跑）：

| | 結果 |
|---|---|
| **DrawCall**（路圖 212 顆標記） | 每顆一個 `Graphics` **232** → Sprite 共用圖集加染色 **28**，之後再九刀收到整桌 **10**。對照組留在程式裡，網址加 `?roads=graphics` 可當場切回去比 |
| **加了 shader 的 DrawCall** | 三個自訂 shader 全開仍是 **10**（全桌共用兩份材質，每張牌不同的參數走頂點色）；錯誤示範——**畫面一個像素都不變**，只差每個 Sprite 各自 `getMaterialInstance()`——跳到 **115**。**shader 本身不貴，實例化才貴** |
| **首載**（Asset Bundle 分包） | 四包加總約 8 MB → 主包 **2.33 MB**。點進百家樂才多載 704 KB（bundle 428 KB ＋ 共用貼圖 276 KB），再進輪盤時共用資源**一個 byte 都沒有重載** |
| **進場等待**（載入剖析） | 本機伺服器、冷快取、模擬 700 kbps 線路（CDP 節流 + 40 ms 延遲，文字資源 gzip），量到載入遮罩揭開為止：實際傳輸 2.23 MB → **0.98 MB**，到大廳畫面可見 **31.5 秒 → 15.2 秒**。三刀：裁掉專案沒用到的引擎模組（Spine 的 asm.js 就 354 KB）、照「畫面上實際用得到多大」重壓大廳美術（1417→453 KB）、`preload` 讓引擎不必排在那串小檔案後面（開始下載的時間點 2697 ms → 463 ms）。後續再修掉原本會 404 的 `src/index.js` preload，並把等待畫面換成一張 **40 KB** 的共用美術 |
| **資源釋放** | 大廳↔三款進出三輪，`assets` 113 / `Texture2D` 25 / `SpriteFrame` 25 — 三輪完全沒有成長 |

上線後照同樣方式量正式站，才發現**瓶頸已經不在自己身上**：傳輸量對得起來（1.00 MB），
但把頻寬開到滿仍要 14.5–19.1 秒。那 44 個請求串成一條依賴鏈（import-map → index.js →
settings → bundle → 貼圖），每一跳都得先等上一跳回來才知道要抓什麼。於是把**同一份檔案**
搬到 Cloudflare Pages 再量一次：

| 同一份檔案、同一台機器、同一個瀏覽器 | GitHub Pages | Cloudflare Pages |
|---|---|---|
| 到大廳畫面可見（寬頻，跑三次） | 14.5 / 18.8 / 19.1 秒 | **2.6 / 2.6 / 2.6 秒** |
| 其中網路 ／ CPU | 15.5 秒 ／ 3.3 秒 | **0.5 秒** ／ 1.8 秒 |
| 平均每個請求等回應 | 406 ms | **18 ms** |
| 邊緣節點 ／ TCP 握手（≈ 一次 RTT） | 新加坡 ／ 65 ms | 台北 ／ **7 ms** |
| 243 KB 貼圖下載（跑三次） | 0.37 / 1.57 / 2.49 秒 | 0.063 / 0.074 / 0.070 秒 |
| **模擬 700 kbps 窄頻** | 19.1 秒 | 15.1 秒 |

最後一列才是這張表的重點：**CDN 換掉的是延遲與吞吐，不是體積**。在真正的窄頻上
1 MB 就是要走 11 秒起跳，誰也繞不過去——前面那些壓體積的工一點都沒白費，
只是它解決的問題跟換託管解決的問題不是同一個。

賭場因此搬到 Cloudflare Pages，樂園與其餘頁面仍在 GitHub Pages（那份賭場也留著當備援）。
回程那顆「← 遊樂園」走的是 Cocos 產物裡寫死的 `../park.html`，用一條 `_redirects` 接回去就通，
**Cocos 原始碼一行沒動**；`sessionStorage` 的返回點跨 origin 來回也不會掉，實測過。
剩下的 1.8 秒是引擎初始化的 CPU 時間，那才是這個專案自己的下一道題。

載入遮罩由 Cocos 主動揭開，不再輪詢旗標：大廳與三款玩法都在必要美術實際套到節點後才呼叫
`finish()`，遮罩本身不隨場景銷毀，所以切場景重用同一個 DOM，那張圖只下載一次。

slot bundle 另做了格式壓縮：兩張不含透明的背景 PNG 改為 JPEG、兩張透明轉軸框改為
WebP，維持原尺寸與 UUID，整包由 **4.27 MB 降到 1.54 MB**（少 2.73 MB／64%）。
