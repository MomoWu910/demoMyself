...
上線後照同樣方式量了正式站：傳輸量對得起來（**1.00 MB**），但進場時間換成由跨海往返主導——
把頻寬開到滿仍要 **14.5 秒**，因為那 44 個請求串成一條依賴鏈（import-map → index.js →
settings → bundle → 貼圖），每一跳都要再等一次 GitHub Pages 的回應（單檔冷連線實測 TTFB 約 450 ms）。
**體積這一刀已經見底，下一刀是砍請求數**（合併 import 檔、把小 json 併進 bundle）。
