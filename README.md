...
| **進場等待**（載入剖析） | 冷快取、模擬 700 kbps 線路（CDP 節流，文字資源 gzip），量到載入遮罩揭開為止：實際傳輸 2.23 MB → **0.98 MB**，到大廳畫面可見 **31.5 秒 → 15.2 秒**。三刀：裁掉專案沒用到的引擎模組（Spine 的 asm.js 就 354 KB）、照「畫面上實際用得到多大」重壓大廳美術（1417→453 KB）、`preload` 讓引擎不必排在那串小檔案後面（開始下載的時間點 2697 ms → 463 ms）。後續再修掉原本會 404 的 `src/index.js` preload，並把等待畫面換成一張 **40 KB** 的共用美術 |
