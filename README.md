...
| **進場等待**（載入剖析） | 原優化量測為實際傳輸 2.23 MB → **0.95 MB**；同一條模擬的 700 kbps 線路上，到大廳畫面可見從 **31.5 秒降到 15.0 秒**。三刀：裁掉專案沒用到的引擎模組（Spine 的 asm.js 就 354 KB）、照「畫面上實際用得到多大」重壓大廳美術（1417→453 KB）、`preload` 讓引擎不必排在那串小檔案後面（開始下載的時間點 2697 ms → 463 ms）。本輪另修正原本會 404 的 `src/index.js` preload，並加 **40 KB** 共用載入美術；圖片進大廳時快取，三款切場景不重抓。完整遠端重測前保守預算約 **0.99 MB / 15.5 秒** |
