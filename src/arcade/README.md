...
### 版面：跟數位桌台幾乎一樣，中間那塊換成視訊

路圖在上、座位左右夾住、階段膠囊、注區兩列、操作面板貼底——整個骨架跟 `games/baccarat` 是同一份，連兩套版面的切換門檻（`h <= 560 && w >= h`）都一樣。**唯一換掉的是中間那一塊**：那裡原本是 Pixi 畫的牌，現在是一塊會播放的矩形。

所以注區、飛幣、座位、線上人數、五張路圖、階段膠囊全部是既有元件，`SeatInfo` / `OtherBet` / `SeatResult` 連型別都直接跟數位桌台共用。**這一款要證明的正是這件事**：換一種媒介不必換一套介面，因為玩家坐的是同一種桌子。

唯二調整過的數字，兩個都是「視訊比牌更吃面積」的後果：路圖上限從 172 降到 148，橫放時的路單條從 76 降到 60。牌被壓到 64px 寬還讀得出點數，640×360 的素材被壓到那個尺寸只剩一片綠色。
