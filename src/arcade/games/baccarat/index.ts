...
export class BaccaratModule implements GameModule {
     ...
     * 3. 橫向：**由牌位框反推，不是由牌堆反推**。莊閒兩堆各偏離中線 1.85 倍牌寬，
     *    而畫面上實際佔寬的是外面那個框（`DealSpots` 的 `2.24 + PAD_RATIO`＝2.46 倍），
     *    合計 3.7 + 2.46 = 6.16 倍牌寬，留一成的邊 → stage.w / 6.78 ≈ stage.w * 0.147。
     *
     *    這裡原本寫的是 0.19，那是拿「牌堆本身 1.12 倍」算出來的——**框比牌堆寬一倍多**，
     *    於是整組牌位比中央區還寬 18%。橫屏看不出來（那裡牌寬是被 CARD_MAX_W 或高度
     *    卡住的，輪不到橫向上限），直屏才露餡：兩個框各被畫面切掉一截。
     */
    private placeCards(stage: Rect): void {
        this.cardW = Math.max(40, Math.min(CARD_MAX_W, (stage.h - TOTAL_LABEL_H) / 2.5, stage.w * 0.147));
